...
**Built at task 3.6a, and it is a chip in the open row rather than a toggle in the panel.** The task that claimed it was written about the insights panel rather than about the filter bar — *make the never-worn count a link to the list behind it* — and the toggle is what the link needed to land on, which is how a line deferred to Stage 5 was closed from Stage 3 instead. It sits after `[Filters ▾]` and not inside it: the panel is shut by default, and a control nobody can see would leave a narrowed grid with nothing on screen saying why. It carries **no count**, unlike the nine category chips — the number is on the insights panel two rows above, counted by the server over the whole wardrobe, where this row can only see the loaded page.

**The filter reads `status`, and it is the only one that does.** `worn` and `never_worn` are scoped to `ready` rows, so the list behind the count has to be scoped the same way or it answers *which ones are they?* with `processing` and `failed` tiles whose `wear_count` is 0 because the column defaults to 0. Every other filter here is deliberately status-blind — that is what keeps a tagging row on screen under a filter it cannot match — and `DECISIONS.md` **228** carries why that rule survives this exception.

**The count line on the insights panel is the link, and the whole sentence is the anchor.** Linking the numerals alone would mean splitting one i18n key into fragments, which the panel already refuses on the grounds that a sentence assembled from pieces cannot be reordered for a language that words it differently. The all-worn sentence is **not** a link: there is no list behind it. The link drops any filter already on rather than intersecting with it, because the count it states is over the whole wardrobe.

**And it moved how this screen reads its own URL.** The link navigates from `/wardrobe` to `/wardrobe`, so the route config does not change, the page component is reused, and the single `route.snapshot` read `DECISIONS.md` 110 specified never runs a second time. `WardrobePage` subscribes to `queryParamMap` instead — `DECISIONS.md` **229**. The panel is unchanged in what it holds: still no store, still fetching for itself.
