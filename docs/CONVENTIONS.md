...
Scopes: `api` · `web` · `ai` · `db` · `auth` · `core` · `storage` · `weather` · `sets` · `e2e` · `ci` · `docs`

A scope names the module a reader would grep for, not the deployable unit. `auth`, `core`, `storage` and `weather` were added at task 0.5 because four stage files already used them in their commit checkpoints and the list here did not — widening the list is one line, whereas narrowing it would mean renaming things inaccurately (`feat(db): closed vocabulary enums` is false; `enums.py` touches no database). `sets` was added the same way before task 4A.1: that task spans a migration, five routes and a line of the stylist prompt, so no existing scope is true of two thirds of its diff.
...
Backend returns `{ "detail": str, "code": str }`. Codes are stable strings the frontend can branch on: `wardrobe_too_small`, `trip_too_long`, `stylist_failed`, `forecast_unavailable`, `geocoding_unavailable`, `home_location_missing`, `destination_not_found`, `anchor_unavailable`, `locked_unavailable`, `set_item_unavailable`, `set_member_taken`, `rate_limited`, `item_edited`, `email_exists`, `invalid_credentials`, `invalid_token`, `validation_error`, `unsupported_file_type`, `file_too_large`, `not_found`, `upload_failed`.
...
**`destination_not_found` (`400`) was added at task 4.4**, `POST /trips/pack` and `POST /trips/{id}/repack`, when the geocoder **answers** and nothing matches what the user typed. It is the nineteenth code and it sits with `wardrobe_too_small` and `home_location_missing` rather than with `geocoding_unavailable`, because it is a statement about the request's content and not about a provider: `services/geocoding.py` has kept the two apart since 2.2 — `GeocodingError` for a provider that did not answer, an empty list for a query that matched nothing — and `DECISIONS.md` 152 is the argument for keeping them apart on the wire too. Reusing `geocoding_unavailable` would tell a user to try again shortly for a spelling that will never resolve. `400` rather than `422`: the body is well formed and the schema has nothing to say about it, and the only thing that could have caught it earlier is the geocoder itself. The message names the string that failed and points at the autocomplete, which is where a correct client picks its destination (`STAGE-4` 4.5). The code has no `DECISIONS.md` entry of its own — it was named at 4.4's orientation and approved before it was written anywhere, and this paragraph is its record; `DECISIONS.md` 202 carries the one consequence that is not obvious, which is that a **repack** can raise it for a trip that packed cleanly a week earlier.
...
**`set_item_unavailable` (`422`) was added at task 4A.1**, for `POST /sets` and `POST /sets/{set_id}/items` when an id names nothing in this account's wardrobe that could be styled — another account's item, or one this account owns that is still `processing` or `failed`, archived, or in an excluded category. It is `anchor_unavailable`'s check a third time, and the same widening: a set exists so that the stylist prefers two garments together, so a garment the stylist is never shown cannot be a useful member of one. Refusing it here costs one lookup and answers plainly, where accepting it writes a relation that quietly does nothing — no `502` this time, because a set makes no model call of its own, which is why the argument is about a silent no-op rather than about a wasted call. It is a separate code from `validation_error` because it is the `422` on these endpoints a correct client can provoke: the user tapped **Add to this set** on a real garment, one that can have been archived from another tab since the picker was drawn. The client's sentence names the garment and says why it cannot join, where *check your request* would be about a request the user never made.

**`set_member_taken` (`422`) was added at the same task**, for the same two endpoints, when an id names an item that already belongs to **another** set. `02-DATA-MODEL.md` makes membership one-to-many through a single nullable `items.set_id`, so a second set cannot hold the same garment and the column is not what refuses it — the API is. It is a second code rather than a reuse of `set_item_unavailable` on the same reasoning that kept `locked_unavailable` separate from `anchor_unavailable` (`DECISIONS.md` 177): the frontend branches on the code to choose a message, and the two situations need different ones. *This garment can't be styled* and *this garment is already in another set* lead to different next actions, and only the second one has an obvious one — open that set. It is also a correct client's `422`: the picker shows the wardrobe, and nothing on a tile says which garments are spoken for. Adding an item to the set it is **already** in is not this code and not an error at all — the request asked for a state the set is in, and a double-tap is not a failure. `DECISIONS.md` 230.
