...
**Amended at 1.9: "never a free-text input" is about tags, and `display_name` is not a tag.** Read literally the sentence removes the only field a select cannot produce — the alt text, the grid's accessible name, the only human-readable thing on a row with no tags, and the field 1.11 plans to mine for vocabulary words the schema discarded. It ships as a text input, trimmed, with a blank sent as `null`. The ten closed-vocabulary fields are selects as written; `formality` and `warmth` take 1.8's range controls and `water_resistant` a checkbox, none of which is a select either. `DECISIONS.md` 125.
