...
**Auth pages:** single-column and centered, which they already are — `mx-auto flex min-h-dvh w-full max-w-sm flex-col justify-center` **stays**, and only the padding string is replaced (`p-6` → `px-6 py-16`). An earlier draft's bare `max-w-sm mx-auto py-16 px-6` would have dropped the vertical centring and anchored the form under the viewport top, which reads badly on a tall screen and has no nav bar above it to justify the position — the bar renders only when authenticated.

Form inputs take the pass's **shared field string verbatim** — `min-h-11 rounded-md border border-line-strong bg-surface px-3 text-sm focus:border-accent focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent` — the same one the tag editor, the stylist form and the trip form carry. An earlier draft wrote `h-11`, which fixes the height where 05 line 652 makes 44px a floor; having one string is the point of having one. Submit is the primary `Button`.

**The bootstrap notice is the restore notice, and it has two branches.** `login.page.spec.ts` names it (`describe('the bootstrap notice')`) and asserts a class-level asymmetry inside it: the signed-out paragraph carries `text-danger` and the unreachable one deliberately does not (`DECISIONS.md` 057). Both branches get a wrapper — `bg-surface-elevated rounded-md p-3 text-sm`, **no colour on the wrapper**, so each paragraph keeps its own. No card border; the surface-elevated colour is the distinction. The demo-wardrobe button becomes a `Button` (secondary), its hint moves from `opacity-70` to `text-ink-muted`, and the notice's Retry becomes a ghost `Button` — it was already an underlined text button, which is what that variant is.

**Profile page:** header pattern matches Stylist — uppercase caption (`profile.caption`) above an h1 Fraunces. Save is primary `Button`.

**Four sections, and they have to be made rather than re-skinned.** The form is one `<section>` (home) plus loose labels, so "sections separated by `border-bs border-line pt-6`" means choosing the groups: **identity** (display name, height), **sizes**, **style notes**, **home**. The first carries no border. They are divided rather than headed — every control already has its own label, and a heading over each group would demote all of them to subtitles.

**Three treatments this screen inherits from its twins**, none of which the bullet named and all of which are the same code one or two screens over: `fieldClass` moves off the pre-token `border border-current/20` string onto the shared field string (DR.5 made the identical change in `trip-form.ts`); the style-notes textarea takes it too; the home-city chip becomes `rounded-xl … shadow-sm`, the third instance of the block behind the stylist's anchor pin and the trip form's chosen destination; and the two `opacity-70` hints become `text-ink-muted`, which is the token that did not exist when they were written.

**Saved looks page:** h1 to `font-display text-4xl leading-tight tracking-tight`, like every other feature page. **No caption** — the stylist's is a dateline, the trip form's is `trip.caption`, and this screen has no second thing to say that its title does not already say; inventing a string to fill the slot would be pattern-matching rather than design. Empty state uses the `EmptyState` component with copy pointing at the stylist as the CTA, projected as an `<a appButton>` since it navigates.

**The look rows are in scope too.** The bullet covered only the empty state, which would have left this screen carrying the one heart toggle in the product that is not the `Button` the stylist card's is, and a hand-rolled `bg-accent` wear button beside five screens of `[appButton]` primaries. The rows become `rounded-2xl … shadow-sm` to match the trip look, the heart takes `variant="secondary"` exactly as the stylist card's does, and "I wore this" becomes a primary `Button`.

**Nav bar:** the active item becomes a pill — `routerLinkActive="bg-accent-wash font-medium text-accent"` (from the current `font-medium text-accent`; the draft's `rounded-md` is dropped as a duplicate, the anchor's static class already has it).

**Inactive items get `text-ink-muted` from a class binding, not a second static class.** They have no colour today — they inherit `--color-ink` — so this is an addition, and `text-accent` and `text-ink-muted` set the same property: two utilities for one property are settled by the order of the compiled stylesheet rather than by the order they are written, which is not a coin this pass should flip. The anchor exports the directive (`#active="routerLinkActive"`) and binds `[class.text-ink-muted]="!active.isActive"`, so the inactive colour is simply absent whenever the active one is present. The sign-out button stays underlined ghost — as it is, not as a `Button`, whose `px-5` is wrong in a bar that already scrolls at 390px.

**index.html boot state:** the `.boot-mark` class stays Fraunces (it must — nothing else has loaded). `.boot-hint` has no `color` at all today — it inherits `.boot`'s — so this **adds** a declaration rather than changing one: `color: var(--color-ink-muted, #5a5a56)`, the `var(--token, literal)` idiom the line above it already uses, so the token wins once the theme has loaded and the literal covers the frames before it.
