...
**Both words entered the vocabulary at task 2.6a, and the filter that reads them has not been built yet** — `Category` has nine members, `SUBCATEGORIES` gives each of the two a list, and migration `0003` taught the `item_category` type both labels. That is half of `AUDITS.md` **O-21**; the other half is the filter, which belongs to task 2.7 beside `ready`-only and `is_archived`. `services/stylist.py` filters nothing and serialises exactly the wardrobe it is handed, which is what lets it be tested as a format. So the sentence above is no longer describing a filter that *cannot* run — it is describing one that is not written yet.
